				...
				for _,wheel in pairs(vehicle.spec_wheels.wheels) do
					for ChildIndex=1,TotalNumberOfChild do
						...
						if wheel.driveNode == ActChild then
							...
							-- Get largest radius
							local r = wheel.radiusOriginal or wheel.radius or 0
if r > LargestWheelRadius or LargestRadius > LargestWheelRadius then
    LargestWheelRadius = r
end
...
function REAcog:CalcWheelForcePoint(wheel)
	...
	local r = wheel.radius or wheel.radiusOriginal or 0
    local forcePointY = positionY - r * HeightFactor;
...
function REAcog:updateMass()
    ...
    for _, component in ipairs(self.components) do
        if component.lastMass == nil then component.lastMass = 0 end
